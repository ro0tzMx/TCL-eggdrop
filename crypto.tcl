...
proc cryptocheckpub {nick host handle channel text} {
  ...
  if {$text == ""} {
    variable ccVersion
    msg $channel "$ccVersion - Usage: !crypto <bitcoin/ethereum/ripple/etc...> (Puedes solicitar varios, separador por un espacio)"
    return
  }
  # Clean input
...
proc cryptocheckmsg {nick uhost handle text} {
  ...
  if {$text == ""} {
    variable ccVersion
    msg $nick "$ccVersion - Usage: !crypto <bitcoin/ethereum/ripple/etc...> (Puedes solicitar varios, separador por un espacio)"
    return
  }
  # Clean input
  set text [string trim $text]
