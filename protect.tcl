...
#
# * You can use mode lock modes such as "M-k type.flood" which will work fine with AllProtection.
#
...
lappend ap:udefs {ap:textc "550:3 120 k:kb 2"}

set textc(kmsg) "Por Favor evita el Flood... $kckcount(form)"
...
set repeatf(kmsg) "Por Favor evita el Flood... $kckcount(form)"
...
set joinflood(lockmode) "M-k clone.join.flood"
...
lappend ap:udefs {ap:massjoin "9:3 M-k join.flood 60"}
...
lappend ap:udefs {ap:brevdoor "5:3 M-k revdoor.flood 60"}
...
lappend ap:udefs {ap:bpartmsg "5:3 M-k partmsg.flood 60"}
...
lappend ap:udefs {ap:bcodes "25:3 M-k codes.flood 60"}
    ...
    if {!$sfluds} {
        if {[string length $apfp(rmode)]==1} {
            if {!$bred} {
                ...
                if {$apfp(rtime) > 0} {
                    utimer $apfp(rtime) [namespace current]::reM
                }
                set bred 1
