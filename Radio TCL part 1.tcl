set adminchans "#channel admin radio"
set adminchans2 "#channel admin radio 2"
set djadminchans "#channel djs"
set chans ""
...
if {([lsearch -exact [string tolower $djadminchans] [string tolower $chan]] != -1) || ($djadminchans == "")} {
   set host $text!*@*
   set master "root"
   set mensajeRoot " 1»10R1« 10R1adio-10M1ars Recibe A Su 10Root 1Con Orgullo 10:6†10: $text 10:6†10:"
   if {[file exists $addfile]} {		       
	  set in [open $addfile r]			   
	  	while {![eof $in]} {					   
		    set vline [gets $in]                           
...
if {([lsearch -exact [string tolower $djadminchans] [string tolower $chan]] != -1) || ($djadminchans == "")} {
   set host $text!*@*
   set master "djs"
   set mensajeDj " 1»10R1« 10R1adio-10M1ars Recibe A Su 10Dj 1Con Orgullo 10:6†10: $text 10:6†10:"
   if {[file exists $addfile]} {		       
	  set in [open $addfile r]			   
	  	while {![eof $in]} {					   
		    set vline [gets $in]                           
